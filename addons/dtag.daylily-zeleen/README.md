...
### Step2: Generate Tag Definitions

Use "Project -> Tools -> Generate DTag Definitions" to generate necessary data files.

> **⚠️ Important:** You must manually run this step after editing any `.dtag` file, otherwise the inspector selectors and runtime redirect will not reflect the latest definitions.

By default, a GDScript constants file `res://dtag_def.gen.gd` (`DTagDef`) is generated for convenient code access — this is the **default code generator**. You can also add custom code generators to generate definitions for other languages (see [section 4](#4-adding-custom-code-generators)).

This is what the generated script looks like from the "example.dtag" in step1:
...
After generation, tag constants are accessible through the auto-generated `DTagDef` class.
...
Still, you can also obtain tag values without `DTagDef` by using the exported inspector properties (see Advanced below).
...
Custom code generators must be marked as tool scripts and have a generation function with the signature `func generate(parse_result: Dictionary[String, EntryDef], redirect_map: Dictionary[String, String]) -> String`, returning the generated file path.

> `EntryDef` is the abstract base class of `TagDef` and `DomainDef`, imported from `res://addons/dtag.daylily-zeleen/editor/parser.gd`.
